...
func (w *Write) MarshalJSON() ([]byte, error) {
	if w.Key != nil {
		return json.Marshal(&struct {
			...
			Key: 		fmt.Sprintf("%s (%d)", w.Key.Name, w.Key.Id),
			Object: 	fmt.Sprintf("%s (%d)", w.Object.Name, w.Object.Id),
			Service:	w.Service,
...
func (r *Read) MarshalJSON() ([]byte, error) {
	return json.Marshal(&struct {
		...
		ReadKey: 	fmt.Sprintf("%s (%d)", r.Key.Name, r.Key.Id),
		Service: 	r.Service,
...
func (write *Write) String() string {
	if write.Key != nil {
		return fmt.Sprintf("{ %s(%d):%s(%d) @ %s by %s }", write.Key.Name, write.Key.Id, write.Object.Name, write.Object.Id, write.Database.GetName(), write.Service)
	}
	return fmt.Sprintf("{ :%s (%d) @ %s by %s }", write.Object.Name, write.Object.Id, write.Database.GetName(), write.Service)
}

func (read *Read) String() string {
	return fmt.Sprintf("{ :%s (%d) by %s - WRITE: %s}", read.Key.Name, read.Key.Id, read.Service, read.Write.String())
}
