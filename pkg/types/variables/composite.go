...
func (v *CompositeVariable) DeepCopy(force bool) Variable {
...
func (v *CompositeVariable) AddReferenceWithID(target Variable, creator string) {
	...
	logger.Logger.Tracef("[VARS COMPOSITE] added reference (%s) -> (%s) with id = %d (creator: %s)", v.VariableInfo.Name, target.GetVariableInfo().GetName(), v.VariableInfo.Id, creator)
}
